"""Base model class for Milvus models."""
...
from pymilvus import DataType, FieldSchema, Hit
...
class MilvusModelMeta(ABCMeta):
    ...
    _models: Dict[str, Type['BaseMilvusModel']] = {}
    
    def __new__(mcs, name, bases, namespace, **kwargs):
        # Add base fields if not already present
        if name not in ['BaseMilvusModel', 'MilvusModel']:
        ...
        if name in ['BaseMilvusModel', 'MilvusModel']:
            return cls
    ...
    @classmethod
    def get_model(mcs, name: str) -> Optional[Type['BaseMilvusModel']]:
        """Get a registered model by name."""
        return mcs._models.get(name)
    
    @classmethod
    def get_all_models(mcs) -> List[Type['BaseMilvusModel']]:
...
class BaseMilvusModel(ABC, metaclass=MilvusModelMeta):
